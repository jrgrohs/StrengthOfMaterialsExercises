...
def server(input, output, session):
    ...
    def ui_problem_statement():
        return[ui.markdown(f"A single force F = {F()} kips pulls on three cylindrical rods, each of length L = {L()} in.Diameter d[1] = {d1()} in. and d[2] = {d2()} in. What is the strain in the steel cylinder? Assume E[steel] = 29,000 ksi and E[aluminum] = 10,000 ksi.")]
    
    @reactive.Effect
    @reactive.event(input.generate_problem)
    def randomize_vars():
        random.seed(input.ID())
        F.set(random.randrange(2, 30, 0.1))
        L.set(random.randrange(5, 20, 0.1))
        d1.set(random.randrange(1, 5, 0.1))
        d2.set(d1()/2)
