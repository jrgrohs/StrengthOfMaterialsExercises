...
def server(input, output, session):
    ...
    def ui_problem_statement():
        return[ui.markdown(f"A bar is attached to two wires, one steel and one aluminum. If the lengths of the wires L[1] = {L1()} in. and L[2] = {L2()} in., find the distance x that load F = {F()} kips must be placed at so that the bar remains horizontal after the load is applied. Both wires have the same cross-section area A = {A()} in.^2. Assume E[steel] = 29,000 ksi and E[aluminum] = 10,000 ksi.")]
    ...
    def randomize_vars():
        ...
        L2.set(L1()*2)
        F.set(random.randrange(3, 15, 0.1))
        A.set(random.randrange(.02, .25, 0.01))
