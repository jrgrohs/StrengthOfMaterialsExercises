...
def server(input, output, session):
    ...
    def ui_problem_statement():
        return[ui.markdown(f"A plastic cylindrical peg is constrained by a metal cap as shown. An axial load of F = {F()} lb is applied to the peg. If d<sub>1</sub> = {d1()} in and d<sub>2</sub> = {d2()} in, determine the normal stress in the peg. Assume the axial load is evenly distributed across the peg and that the metal cap is fixed and does not move.")]
